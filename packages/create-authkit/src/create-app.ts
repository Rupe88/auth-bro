...
import { SchemaGenerator } from '@rupeshe/auth-bro';
...
async function getProjectConfig(): Promise<Omit<ProjectConfig, 'name'>> {
  ...
  return config as Omit<ProjectConfig, 'name'>;
}

async function setupDatabase(projectPath: string, config: Omit<ProjectConfig, 'name'>): Promise<void> {
  const dbPath = path.join(projectPath, 'prisma');

  // Generate Prisma schema
  const schema = SchemaGenerator.generate(config.database as any);
...
async function generateEnvFile(projectPath: string, config: Omit<ProjectConfig, 'name'>): Promise<void> {
  const envContent = SchemaGenerator.generateEnvTemplate(config.database as any);

  await fs.writeFile(path.join(projectPath, '.env'), envContent);
}

async function generateDockerCompose(projectPath: string, config: Omit<ProjectConfig, 'name'>): Promise<void> {
  const dockerContent = SchemaGenerator.generateDockerCompose(config.database as any);

  await fs.writeFile(path.join(projectPath, 'docker-compose.yml'), dockerContent);
}

function showNextSteps(projectName: string, config: Omit<ProjectConfig, 'name'>): void {
